...
2. Confirm the change belongs to the active roadmap phase and does not contradict an accepted
   decision.
...
Use [`docs/DEVELOPMENT_WORKFLOW.md`](docs/DEVELOPMENT_WORKFLOW.md) for branch, worktree, and
integration conventions. Automated contributors also follow [`AGENTS.md`](AGENTS.md); Claude Code
uses the shared adapter in [`CLAUDE.md`](CLAUDE.md).
...
PostgreSQL and MinIO are required for integration tests:
...
- `demonstrated`: a reproducible artifact or operational experiment supports the claim;
