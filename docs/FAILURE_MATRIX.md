...
| F-056 | Experiment mutation uses a stale expected version or reuses an idempotency key for different content | Assertion, validation, Passport, or Package command | Stable typed conflict; no new version or release | All prior versions and releases retained | Refresh the aggregate or use the original request for a true replay | Application, HTTP contract, and PostgreSQL concurrency tests |
| F-057 | Experiment validation contains a blocking finding | Passport release | Deterministic blocked release decision; no released Passport or Package | Experiment version and findings retained | Add evidence through an append-only user assertion, then validate the new version | Blocker/warning/unknown report and transition tests |
| F-058 | Downloaded Experiment Package has a missing, changed, unexpected, duplicate, unsafe, or checksum-mismatched member | Independent package verification | Stable verification failure; package is not accepted | Downloaded bytes retained for diagnosis; released stored object unchanged | Retrieve the immutable released Package again or investigate storage integrity | Offline tamper matrix and JSON/HTML/lineage contract tests |
| F-059 | Package publication stops after object upload but before database commit | Experiment Package publication | Package object metadata remains `pending`; no Package release row is committed | Uploaded bytes retained as pending evidence | Reconcile by recorded SHA-256; retry idempotently | PostgreSQL/MinIO integration test at the publication boundary |
| F-060 | Gamry DTA names an unsupported Framework version, technique, table schema, or multiple/mixed table objects | Vendor parser | Typed `dta_unsupported_*` or `dta_mixed_technique_blocks`; no normalised observation is created | Exact source artifact and rejected parser record retained | Add a reviewed versioned parser variant or provide an explicitly supported source; never select a block heuristically | Offline variant matrix plus PostgreSQL retrieval test for the rejected parser record |
| F-061 | Gamry DTA encoding, decimal convention, row count, row length, or numeric cell conflicts with the profile | Vendor parser | Typed location-aware DTA diagnostic; no partial observation is accepted | Exact source artifact and rejected parser record retained | Correct the explicit profile or submit corrected source bytes as a new artifact; never detect locale or remove rows silently | Offline encoding, BOM, locale, malformed, truncated, missing-value, and finite-number tests |
| F-062 | Retained parser record content or identity is changed | Parser-record retrieval or Package verification | Typed parser-record integrity or `package_lineage_open` failure | Source, prior parser record, observation, and Package bytes remain retained | Quarantine and investigate; never refresh the parser identity or Package manifest silently | Canonical-identity tests, PostgreSQL retrieval validation, and Package parser-substitution test |
| F-063 | Package schema `2` omits the parser record, parser producing version, or parser-to-source/profile/observation/graph link | Independent Package verification | Stable Package schema or lineage failure; Package is not accepted | Downloaded Package bytes remain available for diagnosis | Rebuild from retained evidence with the complete parser member and producing versions | Offline Package v2 closure and tamper tests; legacy generic CSV Packages remain schema `1` |
| F-064 | Electrolysis profile omits or duplicates time, current/current-density, or potential mapping, or lacks an explicit unit | Generic electrolysis ingestion | Typed profile or parser failure; no normalised observation is created | Primary source artifact remains retained | Correct the explicit profile and retry as a new content identity | Offline profile, parser, unit, and structural tests |
| F-065 | Auxiliary analytical declaration lacks the exact electrical source, retained analytical source bytes, sample or collection point, declared method/version, source location, quantity kind, value, or unit | Electrolysis normalisation or Package verification | Typed validation or lineage failure; the result and Package are not accepted | Electrical source and any available auxiliary source remain retained | Supply the complete linked source, sample, collection point, and method declaration | Offline source-link and Package tamper tests plus PostgreSQL/MinIO missing-source, quarantine, rollback, reconciliation, and retry tests |
| F-066 | Conversion, selectivity, yield, or Faradaic efficiency is asserted without an approved equation, complete inputs, units, analysis version, and provenance | Electrolysis validation | Blocking finding; no released Passport or Package | Electrical time series and assertions remain retained | Introduce a reviewed versioned derivation contract or remove the unsupported claim through append-only correction | Conditional validation tests; dimensional and lineage tests are mandatory for any future approved derivation |
| F-067 | Appending a user assertion would produce an experiment version that violates an aggregate invariant, such as repeating an assertion whose content identity already exists | Assertion command | Typed rejection before persistence; no new version | All prior versions and their assertions retained | Supersede the active assertion with a corrected value, or keep the recorded declaration | Domain test that the repeat is refused and that an appended version validates as an aggregate; HTTP contract test that the response carries the stable message rather than a validator dump |
